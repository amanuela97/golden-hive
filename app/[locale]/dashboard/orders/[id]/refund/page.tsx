...
export default async function RefundPage({ params }: RefundPageProps) {
        ...
        <div className="space-y-6">
          ...
          <Card className="p-12">
            <div className="flex flex-col items-center justify-center text-center space-y-4">
              <AlertCircle className="h-16 w-16 text-muted-foreground" />
              <div>
                <h2 className="text-2xl font-semibold mb-2">Cannot Refund Order</h2>
                <p className="text-muted-foreground">
                  This order cannot be refunded. It may not be paid, or it may be archived or canceled.
                </p>
              </div>
            </div>
          </Card>
  ...
  return (
    <DashboardWrapper userRole={roleName}>
      <RefundPageClient orderData={result.data} />
    </DashboardWrapper>
  );
}
