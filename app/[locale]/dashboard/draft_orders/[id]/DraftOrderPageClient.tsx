      ...
      {isCompleted && (
        <Alert className="bg-green-50 border-green-200 text-green-900">
          ...
          <AlertDescription className="flex items-center gap-2 flex-wrap">
            ...
            {completedAt && (
              <span>
                Order created on {format(new Date(completedAt), "MMM dd, yyyy, h:mm a")}.
              </span>
            )}
            <span>You can</span>
            {convertedToOrderId ? (
              ...
              <Table>
                ...
                <TableBody>
                  {initialData.lineItems.map((item) => {
                    ...
                    const productImage = item.variantImageUrl || item.imageUrl || null;
                    return (
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-1">
                              <div className="font-medium">{item.listingName}</div>
                              {item.variantTitle && item.variantTitle !== item.listingName && (
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex justify-between text-sm">
                <span>
                  Subtotal ({initialData.lineItems.length} item{initialData.lineItems.length !== 1 ? "s" : ""})
                </span>
              ...
              <div className="border-t pt-3 flex justify-between font-semibold">
                <span>Total</span>
                <span>{currency} {parseFloat(totalAmount).toFixed(2)}</span>
              </div>
            </CardContent>
          </Card>

          {/* Timeline */}
          {convertedToOrderId && Array.isArray(orderEvents) && orderEvents.length > 0 && (
            <OrderTimeline
